...
def test_shops_to_prices_get_multi(test_client, shop_with_products):
    response = test_client.get(f"/api/shop_endpoints-to-prices")
...
def test_shop_to_price_get_by_id(test_client, shop_to_price_1):
    response = test_client.get(f"/api/shop_endpoints-to-prices/{shop_to_price_1.id}")
    assert HTTPStatus.OK == response.status_code


def test_shop_to_price_get_by_id_not_found(test_client, shop_to_price_1):
    response = test_client.get(f"/api/shop_endpoints-to-prices/{uuid.uuid4()}")
...
def test_shops_to_prices_create_with_nonexistent_product(
    ...
    response = test_client.post(
        f"/api/shop_endpoints-to-prices", data=json_dumps(body), headers=superuser_token_headers
    )
...
def test_shops_to_prices_update(test_client, shop_to_price_2, product_2, superuser_token_headers):
    shop_modified_at = test_client.get(
        f"/api/shop_endpoints/cache-status/{shop_to_price_2.shop.id}", headers=superuser_token_headers
    ).json()["modified_at"]
    body = {
    ...
    response = test_client.put(
        f"/api/shop_endpoints-to-prices/{shop_to_price_2.id}", data=json_dumps(body), headers=superuser_token_headers
    )
    assert response.status_code == HTTPStatus.CREATED

    updated_shop_to_price = test_client.get(
        f"/api/shop_endpoints-to-prices/{shop_to_price_2.id}", headers=superuser_token_headers
    ).json()
...
def test_shops_to_prices_delete(test_client, shop_to_price_1, superuser_token_headers):
    response = test_client.delete(
        f"/api/shop_endpoints-to-prices/{shop_to_price_1.id}", headers=superuser_token_headers
    )
    assert HTTPStatus.NO_CONTENT == response.status_code
    shops_to_prices = test_client.get("/api/shop_endpoints-to-prices").json()
...
def test_shop_to_prices_swap(test_client, shop_with_products, superuser_token_headers):
    ...
    response = test_client.patch(
        f"/api/shop_endpoints-to-prices/swap/{shop_to_price.id}?move_up=true", headers=superuser_token_headers
    )
    assert response.status_code == 201

    response = test_client.get(f"/api/shop_endpoints-to-prices/{shop_to_price.id}")
...
def test_shop_to_prices_swap_employee(test_client, shop_with_products, employee_token_headers):
    ...
    response = test_client.patch(
        f"/api/shop_endpoints-to-prices/swap/{shop_to_price.id}?move_up=true", headers=employee_token_headers
    )
    assert response.status_code == 201
...
def test_shop_to_prices_swap_wrong_employee(test_client, shop_with_products, employee_token_headers_2):
    ...
    response = test_client.patch(
        f"/api/shop_endpoints-to-prices/swap/{shop_to_price.id}?move_up=true", headers=employee_token_headers_2
    )
...
def test_shop_to_prices_swap_down_max(test_client, shop_with_products, superuser_token_headers):
    ...
    response = test_client.patch(
        f"/api/shop_endpoints-to-prices/swap/{shop_to_price.id}?move_up=false", headers=superuser_token_headers
    )
    assert response.status_code == 400
