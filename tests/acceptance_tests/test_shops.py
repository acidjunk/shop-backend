...
def test_shops_get_multi():
    response_prd = requests.get(PRD_BACKEND_URI + "shops", headers=PRD_SUPERUSER_TOKEN_HEADERS).json()
...
def test_shops_full():
    response_multi_prd = requests.get(PRD_BACKEND_URI + "shops", headers=PRD_SUPERUSER_TOKEN_HEADERS).json()
